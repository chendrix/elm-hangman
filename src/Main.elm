...
main =
  StartApp.start { model = randomModel, update = update, view = view }
    ...
    SubmitGuess ->
      let
        newGuesses =
          model.guessInput
          |> String.toLower
          |> String.foldl (\guess guesses -> Set.insert guess guesses) model.guesses
      in
        { model | guesses <- newGuesses
                , guessInput <- ""
        }

    UpdateGuessInput input ->
      { model | guessInput <- input }
  ...
  in
    div
      ...
      , input
          ...
          , on "input" targetValue (Signal.message address << UpdateGuessInput)
          , onEnter address SubmitGuess
          ]
          []
      , button
